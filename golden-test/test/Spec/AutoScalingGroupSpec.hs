module Spec.AutoScalingGroupSpec where
...
template =
  (Stratosphere.mkTemplate [autoScalingGroup, launchConfiguration])
    { parameters =
      ...
      description = Just "Auto scaling group example"
    }
  ...
  let autoScalingGroupResource =
        (Stratosphere.AutoScaling.AutoScalingGroup.mkAutoScalingGroup "1" "4")
          { availabilityZones = Just (Stratosphere.RefList "AvailabilityZones"),
            ...
            terminationPolicies = Just ["OldestLaunchConfiguration", "ClosestToNextInstanceHour"]
          }
      resource =
...
spec :: Spec
spec =
  describe "AutoScalingGroupGoldenTests" $ do
    it "AutoScalingGroup Template" $ do
      runGoldenTest "autoscaling" (Stratosphere.encodeTemplate template)
