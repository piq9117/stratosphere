module Spec.LambdaS3CopySpec (spec) where
...
import Stratosphere.IAM.Role.PolicyProperty (PolicyProperty (..))
import Stratosphere.Lambda.Function (CodeProperty (..), Function (..))
import qualified Stratosphere.Lambda.Function
import qualified Stratosphere.Lambda.Permission
import Stratosphere.S3.Bucket
  ( Bucket (..),
...
spec =
  describe "LambdaGoldenTests" $ do
    it "Lambda Template" $ do
      runGoldenTest "lambda-s3-copy" (Stratosphere.encodeTemplate template)
