module Spec.RDSMasterReplicaSpec (spec) where
...
import Stratosphere.RDS.DBParameterGroup (DBParameterGroup (..))
...
spec :: Spec
spec =
  describe "RDSMasterReplicaGoldenTests" $ do
    it "RDS Master-Replica Template" $ do
      runGoldenTest "rds-master-replica" (Stratosphere.encodeTemplate template)
